...
expected_names <- c(
    "htslib_include", "htslib_lib", "bcftools_lib",
    "cflags", "htslib_libs", "htslib_libs_static",
    "bcftools_libs", "all_libs"
)
expect_true(
...
'
    ...
    # Try to compile
    compile_result <- system(compile_cmd, ignore.stdout = TRUE, ignore.stderr = TRUE)

    # Clean up
    if (file.exists(test_c_file)) file.remove(test_c_file)
    if (file.exists(test_exe)) file.remove(test_exe)

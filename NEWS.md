...
- **`vcf_open_duckdb()`**: Open VCF/BCF files as DuckDB tables or views
  ...
  - `threads` parameter for parallel loading by chromosome (requires `as_view = FALSE`)
...
- **New `partition_by` parameter** for efficient per-sample queries on large cohorts:

...
through operating-system pipes rather than through R strings. On Linux,
`cpu_affinity` starts every concurrent stage under the same `taskset`
CPU set, while the result records native wall time, per-stage peak RSS,
and the largest simultaneous sum of stage RSS sampled from `/proc` every
10 ms.
...
`samtools markdup` input. These examples are executed when the README is
rendered. They build a deterministic reference, map real paired FASTQ
records with both executables, mark duplicates, create indexes, and
query the resulting BAM files. Rendering therefore requires BWA and
Rminibwa rather than silently substituting a mock mapper.
...
#>   reference_bases read_pairs
#> 1           50000         40
```

The BWA stream exercises the full Samtools-compatible duplicate-marking
chain and then indexes the output:
...
explicit file-dependent step rather than a streaming stage. Here the
mapper is the actual executable exported by Rminibwa:
...
exome](benchmarks/hg02088-exome-pipelines/README.md) and on the pinned
[Zenodo HG002 one-million-pair WGS
dataset](benchmarks/hg002-wgs-1m-pipelines/README.md). Both map against
a full human reference with BWA and Rminibwa, then compare Samtools
`markdup` with the patched FastDup on each prepared BAM.

...
documented in [ERRATA.md](ERRATA.md). When compiled with libcurl, remote
...
`pipeline_stage()` and `run_pipeline()` connect arbitrary executables
without constructing a shell command. Arguments are passed directly to
each process, every stage’s status is returned, and binary data moves
through operating-system pipes rather than through R strings.

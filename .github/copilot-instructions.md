...
Currently, structured annotation fields (INFO/CSQ from VEP, INFO/ANN from SnpEff, etc.) are **not specially parsed**—they remain as pipe-delimited or comma-separated strings in the INFO column.

**Rationale**: These annotations have tool-specific schemas that change between versions. Parsing them requires:
1. Reading the header `##INFO=<ID=CSQ,...,Description="...Format: Allele|Consequence|...">`
2. Splitting by `|` and mapping to column names
3. Handling multiple transcript annotations per variant

**Future options** (if needed):
- Use bcftools `+split-vep` plugin via `system2(bcftools_path(), ...)` for VEP
- Add optional `parse_vep = TRUE` parameter to Arrow/DuckDB functions
- Post-process with DuckDB SQL: `SELECT unnest(string_split(INFO_CSQ, '|')) ...`

For now, recommend users run `bcftools +split-vep` upstream or query raw strings with SQL.

...
- [`vcf_to_parquet_tidy()`](https://rgenomicsetl.github.io/RBCFTools/reference/vcf_to_parquet_tidy.md):
  Export VCF/BCF to tidy (long) format Parquet where each row is one
  variant-sample combination
  - Adds `SAMPLE_ID` column and transforms `FORMAT_<field>_<sample>` to
    `FORMAT_<field>`
  - Supports single-sample (simple rename) and multi-sample
    (unnest-based unpivot) VCFs
  - Compatible with `threads` parameter for parallel processing
  - Ideal for combining multiple single-sample VCFs or appending to
    DuckLake tables

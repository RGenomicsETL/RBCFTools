...
```
```{r ducklake-etl, eval=identical(Sys.getenv("DUCKLAKE_RUN"), "1")}
...
if (!nzchar(minio_bin)) {
  minio_bin <- ducklake_download_minio(dest_dir = bin_dir)
}
mc_bin <- Sys.which("mc")
if (!nzchar(mc_bin)) {
  mc_bin <- ducklake_download_mc(dest_dir = bin_dir)
}
...
minio_pid <- system2(minio_bin, c("server", data_dir, "--address", endpoint),
                     stdout = log_file, stderr = log_file, wait = FALSE)
on.exit({ if (minio_pid > 0) system2("kill", as.character(minio_pid)) }, add = TRUE)

Sys.sleep(3)
system2(mc_bin, c("alias", "set", "ducklake_local", paste0("http://", endpoint),
                  "minioadmin", "minioadmin"))
system2(mc_bin, c("mb", "ducklake_local/ducklake-demo"))
...
ext_env <- Sys.getenv("DUCKLAKE_EXT")
if (nzchar(ext_env)) {
  DBI::dbExecute(con, sprintf("LOAD %s", DBI::dbQuoteString(con, ext_env)))
} else {
...
With MinIO running (e.g., `mc alias set myminio/ http://127.0.0.1:9000 MINIOUSER MINIOPASS` and a bucket `mybucket` ready), you can stream variants into DuckLake and query them back:

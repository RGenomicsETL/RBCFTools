...
res <- processx::run(
  bcftools_path(),
  c("view", "-H", "-r", "chr22:20000000-20100000", s3_vcf_uri),
...
# Use processx instead of system2 for MinIO server
if (!requireNamespace("processx", quietly = TRUE)) stop("processx required")
...
on.exit({ if (!is.null(minio_proc) && minio_proc$is_alive()) minio_proc$kill() }, add = TRUE)
...
for (i in 1:30) {
  res <- processx::run(
    mc_bin,
    c("alias", "set", "ducklake_local", paste0("http://", endpoint),
...
for (i in 1:30) {
  res <- processx::run(
    mc_bin,
    c("mb", paste0("ducklake_local/", bucket)),
    error_on_status = FALSE
  )
  if (res$status == 0) {
    bucket_ok <- TRUE
...
if (exists("minio_proc") && !is.null(minio_proc) && minio_proc$is_alive()) {
  minio_proc$kill()
}
```
